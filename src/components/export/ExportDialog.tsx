...
import { ExportManager, ChatExporter, ProjectExporter, ImageExporter, type ExportOptions } from '@/lib/utils/exportUtils'
...
export function ExportDialog({ 
  ...
  const filteredMessageCount = messages?.filter(msg => {
    const dateRange = getDateRange()
    if (dateRange) {
      const msgDate = new Date(msg.created_at)
      if (msgDate < dateRange.start || msgDate > dateRange.end) return false
    }
    
    if (selectedUsers.length > 0 && !selectedUsers.includes(msg.user_id)) return false
    if (!selectedMessageTypes.includes(msg.message_type)) return false
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            📄 Export Data
            <Badge variant="outline">{roofName}</Badge>
          </DialogTitle>
          <DialogDescription>
            Export project data, chat messages, and attachments for compliance and reporting
          </DialogDescription>
        </DialogHeader>
              ...
              <CardContent className="space-y-3">
                <div className="space-y-2">
                  <label className="flex items-center space-x-2 cursor-pointer">
                    ...
                    />
                    <span className="text-sm">Complete Project Export</span>
                    <Badge variant="secondary">Recommended</Badge>
                  </label>
                  <p className="text-xs text-muted-foreground ml-6">
                    Includes project data, pins, messages, and team info
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  <label className="flex items-center space-x-2 cursor-pointer">
                    <input
                      ...
                      className="text-primary"
                    />
                    <span className="text-sm">Project Data Only</span>
                  </label>
                  <p className="text-xs text-muted-foreground ml-6">
                    Roof info, pins, and team members (no chat)
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  <label className="flex items-center space-x-2 cursor-pointer">
                    <input
                      ...
                      className="text-primary"
                    />
                    <span className="text-sm">Chat Messages Only</span>
                  </label>
                  <p className="text-xs text-muted-foreground ml-6">
                    Team communication and attachments
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <Button 
                  variant="outline" 
                  size="sm" 
                  onClick={handleCaptureRoofPlan}
                  className="w-full justify-start gap-2"
                >
                  📸 Capture Roof Plan
                </Button>
                <p className="text-xs text-muted-foreground">
                  Save current roof view with pins as image
                </p>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="space-y-4">
            {(exportType === 'chat' || exportType === 'both') && (
              <>
                <Card>
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription className="text-xs">
                      {messageCount} total messages • {filteredMessageCount} will be exported
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-3">
                ...
                <Card>
                  <CardHeader className="pb-3">
                    <CardTitle className="text-sm">Message Types</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-2">
                    {[
                      ...
                      <div key={type.value} className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            if (checked) {
                              setSelectedMessageTypes([...selectedMessageTypes, type.value])
                            } else {
                              setSelectedMessageTypes(selectedMessageTypes.filter(t => t !== type.value))
                            }
                          }}
                        />
            ...
            <Card>
              ...
              <CardContent className="space-y-2 text-xs text-muted-foreground">
                <div className="flex justify-between">
                  <span>Pins:</span>
                  <span>{pins?.length || 0}</span>
                </div>
                <div className="flex justify-between">
                  <span>Messages:</span>
                  <span>{filteredMessageCount}</span>
                </div>
                <div className="flex justify-between">
